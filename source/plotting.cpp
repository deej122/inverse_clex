...
int main(int argc, char *argv[])
{
	...
	//If not filename given in original argument (./execute_mc filename)
	if (argc > 1){
		filename = argv[1];
	}
	else{
		cout << "In order to run this program you must provide the name of a properly formatted input file." << endl << "Your input file should be written in standard JSON format." << endl << "Please input the name of your input file here: ";
		cin >> filename;
	}

	//If file given doesn't exist
	if (stat(filename.c_str(), &fileCheck) != 0){
		cout << "Sorry, that file doesn't seem to exist. Check that a file of the name you provided is in this directory, or input a new filename: ";
		cin >> filename;
	}

	//Only run function if filename is provided and exists
	if (stat(filename.c_str(), &fileCheck) == 0)
	{
